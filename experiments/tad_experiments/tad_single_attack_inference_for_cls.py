...
for dataset in [
    ...
    for attacker in [
        ...
        text_classifier = TADCheckpointManager.get_tad_text_classifier(checkpoint=f'TAD-{dataset}{attacker}',
                                                                       auto_device=True,  # Use CUDA if available
                                                                       )

        inference_sets = DatasetItem(dataset, findfile.find_cwd_files([dataset, attacker, '.org', '.inference']))
        ...
        text_classifier = TADCheckpointManager.get_tad_text_classifier(checkpoint=f'TAD-BERT-{dataset}{attacker}',
                                                                       auto_device=True,  # Use CUDA if available
                                                                       )
