...
from pyabsa import TCTrainer, TADConfigManager, TCDatasetList, TADBERTTCModelList, TADTrainer
...
def get_config():
    ...
    config.model = TADBERTTCModelList.TADBERT
    config.num_epoch = 30
    config.patience = 5
...
dataset = DatasetItem('IMDB10KBAE')
text_classifier = TADTrainer(config=get_config(),
                             dataset=dataset,
                             checkpoint_save_mode=1,
                             auto_device=True
                             ).load_trained_model()
dataset = DatasetItem('IMDB10KPWWS')
text_classifier = TADTrainer(config=get_config(),
                             dataset=dataset,
                             checkpoint_save_mode=1,
                             auto_device=True
                             ).load_trained_model()
dataset = DatasetItem('IMDB10KTextFooler')
text_classifier = TADTrainer(config=get_config(),
                             dataset=dataset,
                             checkpoint_save_mode=1,
                             auto_device=True
