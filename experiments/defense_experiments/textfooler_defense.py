...
def generate_adversarial_example(dataset, attack_recipe, augmentor):
    attack_recipe_name = attack_recipe.__name__
    sent_attacker = SentAttacker(augmentor.text_classifier, attack_recipe)
            ...
            for text, label in tqdm.tqdm(data, postfix='attacking...'):
                ...
                if result is not None:
                    ...
                    if result.perturbed_result.output != result.original_result.ground_truth_output and \
                        ...
                        augs = augmentor.single_augment(result.perturbed_result.attacked_text.text, result.original_result.ground_truth_output, 11)
                        if augs:
                            infer_results = []
                            for aug in augs:
                                infer_results.append(
                                    augmentor.text_classifier.infer(
                                        aug + '!ref!{}'.format(
                                            result.original_result.ground_truth_output
                                        ),
                                        print_result=True
                                    ))
                            infer_res = get_ensembled_tc_results(infer_results)
                            if int(infer_res) == result.original_result.ground_truth_output:
                                acc_count += 1
                                print(colored('Success', 'green'))
                            else:
                                print(colored('Failed', 'red'))
                            # if infer_res['ref_label_check'] == 'Correct':
                            #     acc_count += 1
                ...
                #     new_data['is_adv'],
                # ))
                # if not os.path.exists(os.path.dirname(data_file) + f'/{attack_recipe_name}/'):
                #     os.makedirs(os.path.dirname(data_file) + f'/{attack_recipe_name}/')
                # fout = open(os.path.dirname(data_file) + '/{}/{}.{}.{}.org'.format(attack_recipe_name, os.path.basename(data_file), i + 1, attack_recipe_name), mode='w', encoding='utf8')
                # fout.writelines(org_data)
                # fout.close()
                #
                # fout = open(os.path.dirname(data_file) + '/{}/{}.{}.{}.adv'.format(attack_recipe_name, os.path.basename(data_file), i + 1, attack_recipe_name), mode='w', encoding='utf8')
...
if __name__ == '__main__':
    ...
    for dataset in datasets:
        ...
        generate_adversarial_example(dataset, attack_recipe=attack_recipes[attack_name.lower()], augmentor=augmentor)
